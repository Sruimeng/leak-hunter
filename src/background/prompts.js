...
export const buildFixPrompt = (leak, context) => {
...
export const buildAnalysisPrompt = (leaks) => {
    const summary = leaks.map(l => `- ${l.descriptor.type} [${l.uuid.slice(0, 8)}] 存活 ${(l.age / 1000).toFixed(1)}s`).join('\n');
